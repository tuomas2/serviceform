...
def copy_serviceform_as_new(service_form: 'ServiceForm') -> 'ServiceForm':
    raise NotImplementedError(
        'not M2M ready (TODO!)! Not secure-auth-link ready! (TODO), Not guardian ready (TODO)')

    from .models import FormRevision
    with transaction.atomic():
        serv_copy = service_form.create_copy()
        serv_copy.name = _('Copy of {}').format(service_form.name)
        serv_copy.slug = service_form._find_new_slug()
        serv_copy.save()
        serv_copy.current_revision = FormRevision.objects.create(form=serv_copy,
                                                                 name=_('new-revision'))
        serv_copy.allowed_users = service_form.allowed_users.all()

        resp_map = {None: None}
        for r in service_form.responsibilityperson_set.all():
            r_copy = r.create_copy()
            r_copy.form = serv_copy
            r_copy.secret_key = generate_uuid()
            r_copy.save()
            resp_map[r] = r_copy

        temp_map = {None: None}
        for t in service_form.emailtemplate_set.all():
            t_copy = t.create_copy()
            t_copy.form = serv_copy
            t_copy.save()
            temp_map[t] = t_copy

        for f in service_form._meta.fields:
            if 'email_to' in f.name:
                setattr(serv_copy, f.name, temp_map[getattr(service_form, f.name)])

        serv_copy.responsible = resp_map[service_form.responsible]
        serv_copy.save()

        for c1 in service_form.sub_items:
            c1_copy = c1.create_copy()
            c1_copy.form = serv_copy
            c1_copy.responsible = resp_map[c1_copy.responsible]
            c1_copy.save()
            for c2 in c1.sub_items:
                c2_copy = c2.create_copy()
                c2_copy.category = c1_copy
                c2_copy.responsible = resp_map[c2_copy.responsible]
                c2_copy.save()
                for a in c2.sub_items:
                    a_copy = a.create_copy()
                    a_copy.category = c2_copy
                    a_copy.responsible = resp_map[a_copy.responsible]
                    a_copy.save()
                    for c in a.sub_items:
                        c_copy = c.create_copy()
                        c_copy.activity = a_copy
                        c_copy.responsible = resp_map[c_copy.responsible]
                        c_copy.save()
        for q in service_form.questions:
            q_copy = q.create_copy()
            q_copy.form = serv_copy
            q_copy.save()
    return serv_copy
